...
func pickup():
	...
	object_ref.call_deferred("reparent", player_hand)
...
func drop():
	...
	if not in_balloon:
		object_ref.call_deferred("reparent", get_tree().current_scene)
	else:
		object_ref.call_deferred("reparent", balloon.balloon_body)
